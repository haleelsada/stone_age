...
def convert_class_to_dict(board: List[List[Any]]) -> List[List[Dict[str, str]]]:
    converted = []
    for row in board:
        new_row = []
        for cell in row:
...
class StudentAgent(BaseAgent):
    ...
    def choose(self, game_state: List[List[Any]],  rows: int, cols: int, score_cols: List[int], current_player_time: float, opponent_time: float) -> Optional[Dict[str, Any]]:
        # board = game_state[0]
        board = convert_class_to_dict(game_state)
        print(board)
        cpp_move = self.agent.choose(board, rows, cols, score_cols, 0.0, 0.0)
        if cpp_move is None:
            return None
